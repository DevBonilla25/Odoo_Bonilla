# Módulo Operaciones Logísticas para Odoo 17

## 📋 Descripción

**Operaciones Logísticas** es un módulo para Odoo 17 que replica la funcionalidad del **Point of Sale (POS)** oficial, pero **reutilizando las mismas tablas de base de datos** existentes.
...
|----------------|---------------------|------------------|
| `pos.config` | `models/pos_config.py` | `x_is_logistic_operations`, `x_source_module` |
| `pos.session` | `models/pos_session.py` | `x_is_logistic_operations`, `x_source_module`, `x_config_is_logistic_operations` |
| `pos.order` | `models/pos_order.py` | `x_is_logistic_operations`, `x_source_module`, `x_session_is_logistic_operations`, `x_config_is_logistic_operations` |
| `pos.payment` | `models/pos_payment.py` | `x_is_logistic_operations`, `x_source_module`, `x_order_is_logistic_operations` |

### Campos Identificadores

Todos los modelos extendidos incluyen estos campos:

#### `x_is_logistic_operations` (Boolean)
- **Propósito**: Campo principal para identificar registros de Operaciones Logísticas
...
- `x_config_is_logistic_operations`: Indica si la configuración asociada es de LSO
- `x_session_is_logistic_operations`: Indica si la sesión asociada es de LSO
- `x_order_is_logistic_operations`: Indica si la orden asociada es de LSO
...
```
pos.config (x_is_logistic_operations = True)
    ↓ hereda
...
#### 1. **pos.config** ([models/pos_config.py:52](odoo/addons/logistic_operations/models/pos_config.py#L52))
```python
@api.model
...
#### 2. **pos.session** ([models/pos_session.py:69](odoo/addons/logistic_operations/models/pos_session.py#L69))
```python
@api.model
def create(self, vals):
...
#### 3. **pos.order** ([models/pos_order.py:99](odoo/addons/logistic_operations/models/pos_order.py#L99))
```python
@api.model
def create(self, vals):
...
#### 4. **pos.payment** ([models/pos_payment.py:48](odoo/addons/logistic_operations/models/pos_payment.py#L48))
```python
@api.model
def create(self, vals):
    if 'pos_order_id' in vals:
        order = self.env['pos.order'].browse(vals['pos_order_id'])
        if order.x_is_logistic_operations:
            vals['x_is_logistic_operations'] = True
            vals['x_source_module'] = 'logistic_operations'
...
Todas las acciones de Operaciones Logísticas incluyen el contexto apropiado:
...
#### 1. `group_logistic_operations_user` ([security/logistic_operations_security.xml:28](odoo/addons/logistic_operations/security/logistic_operations_security.xml#L28))
- **Permisos**:
...
#### 2. `group_logistic_operations_manager` ([security/logistic_operations_security.xml:45](odoo/addons/logistic_operations/security/logistic_operations_security.xml#L45))
- **Permisos**:
  - Todo lo del usuario
...
- **Hereda**: `group_logistic_operations_user` + `point_of_sale.group_pos_manager`

### Control de Acceso a Modelos

Definido en [security/ir.model.access.csv](odoo/addons/logistic_operations/security/ir.model.access.csv):
...
**IMPORTANTE**: Las reglas de registro están **comentadas** por defecto en [security/logistic_operations_security.xml](odoo/addons/logistic_operations/security/logistic_operations_security.xml).

Si quieres separación estricta entre POS y Operaciones Logísticas, descomenta las reglas en el archivo XML.
...
│   ├── logistic_operations_security.xml        # Grupos y reglas
│   └── ir.model.access.csv              # Control de acceso a modelos
│
├── views/                               # Vistas y menús
│   ├── logistic_operations_config_view.xml     # Vistas de configuración
│   ├── logistic_operations_session_view.xml    # Vistas de sesión
│   ├── logistic_operations_order_view.xml      # Vistas de órdenes
│   └── logistic_operations_menu.xml            # Menús principales
│
...
2. **Actualizar la lista de aplicaciones**:
   ...
   ./odoo-bin -u logistic_operations -d tu_base_de_datos
...
1. **Asignar permisos**:
   ...
   - En la pestaña de `Permisos`, asigna:
     - `Usuario Operaciones Logísticas` (para usuarios normales)
     - `Manager Operaciones Logísticas` (para administradores)

2. **Crear primera configuración**:
   - Ve a `Operaciones Logísticas > Configuración > Puntos de Venta`
   - Haz clic en `Crear`
   - Configura igual que un POS normal (almacén, diario, métodos de pago, etc.)
   - Guarda

3. **Abrir sesión**:
   - Ve a `Operaciones Logísticas > Dashboard`
...
1. **Abrir sesión**: `Operaciones Logísticas > Dashboard > Nueva Sesión`
2. **Procesar ventas**: Igual que en POS
3. **Cerrar sesión**: `Operaciones Logísticas > Sesiones > Cerrar Sesión`
4. **Ver órdenes**: `Operaciones Logísticas > Órdenes > Órdenes`
5. **Análisis**: `Operaciones Logísticas > Órdenes > Análisis de Órdenes`
...
1. **Ver campo en formularios**:
   - Abre cualquier orden de Operaciones Logísticas
   - Ve a la pestaña/grupo "Información del Módulo"
   - Verifica que `x_is_logistic_operations = True`

2. **Usar filtros en búsqueda**:
   - En la vista de lista de órdenes
   - Usa el filtro "Órdenes Operaciones Logísticas"
   - Deberías ver solo tus órdenes

### Desde Base de Datos

```sql
-- Ver órdenes de Operaciones Logísticas
SELECT id, name, x_is_logistic_operations, x_source_module
FROM pos_order
WHERE x_is_logistic_operations = true;

-- Ver sesiones de Operaciones Logísticas
SELECT id, name, x_is_logistic_operations, x_source_module
FROM pos_session
WHERE x_is_logistic_operations = true;

-- Ver configuraciones de Operaciones Logísticas
SELECT id, name, x_is_logistic_operations, x_source_module
FROM pos_config
WHERE x_is_logistic_operations = true;
```
...
# En el shell:
# Ver órdenes de Operaciones Logísticas
lso_orders = env['pos.order'].search([('x_is_logistic_operations', '=', True)])
print(f"Total órdenes LSO: {len(lso_orders)}")

# Ver sesiones de Operaciones Logísticas
lso_sessions = env['pos.session'].search([('x_is_logistic_operations', '=', True)])
print(f"Total sesiones LSO: {len(lso_sessions)}")

# Ver estadísticas
stats = env['pos.order'].get_logistic_operations_stats()
print(stats)
```
...
```xml
<!-- En un nuevo archivo views/logistic_operations_reports.xml -->

<record id="action_logistic_operations_custom_report" model="ir.actions.act_window">
    <field name="name">Reporte Personalizado LSO</field>
...
@api.constrains('x_is_logistic_operations', 'partner_id')
def _check_logistic_operations_partner(self):
    for order in self:
        if order.x_is_logistic_operations and not order.partner_id:
            raise ValidationError(
                "Las órdenes de Operaciones Logísticas requieren un cliente"
            )
```
...
### Problema: No veo el menú de Operaciones Logísticas
...
2. Reinstalar: `Apps > Operaciones Logísticas > Desinstalar > Instalar`
...
2. **Rendimiento**: Los campos `x_is_logistic_operations` están **indexados** para evitar impacto en el rendimiento.

3. **Migración**: Si en el futuro decides separar completamente los datos, puedes crear un script de migración basado en `x_is_logistic_operations`.
...
- [x] Campos identificadores agregados (`x_is_logistic_operations`, `x_source_module`)
...
1. **Agregar Dashboard personalizado** con estadísticas específicas de LSO
