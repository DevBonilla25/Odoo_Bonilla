...
class PosPayment(models.Model):
    ...
    Campos agregados:
    -----------------
    * x_is_logistic_operations: Boolean que indica si este pago pertenece a Operaciones Logísticas
    * x_source_module: Selection que indica el módulo de origen
    * x_order_is_logistic_operations: Campo relacionado de la orden

    Lógica de marcado:
    ------------------
    Los pagos heredan automáticamente el marcado de su orden (pos_order_id).
    Si la orden es de Operaciones Logísticas, el pago también lo es.
    """

    _inherit = 'pos.payment'
    _description = 'Point of Sale Payment - Extended for Operaciones Logísticas'

    # Campo booleano identificador
    x_is_logistic_operations = fields.Boolean(
        string='Es Operaciones Logísticas',
        default=False,
        help="Marca si este pago pertenece al módulo Operaciones Logísticas",
        copy=False,
        index=True,
    )
    ...
    x_order_is_logistic_operations = fields.Boolean(
        string='Orden es Operaciones Logísticas',
        related='pos_order_id.x_is_logistic_operations',
        store=True,
        help="Indica si la orden asociada es de Operaciones Logísticas",
    )
    ...
    def create(self, vals):
        ...
        Estrategia:
        -----------
        1. Si pos_order_id está en vals, verificamos si es de Operaciones Logísticas
